"use client"

import type React from "react"
import { useState, useRef } from "react"
import Icon from "./Icon"
import CollapsiblePanel from "./CollapsiblePanel"

interface AccountManagerProps {
  accountCount: number
  onAdd: (accounts: string[]) => void
  onImport: (content: string, fileName: string) => void
  onClear: () => void
}

const AccountManager: React.FC<AccountManagerProps> = ({ accountCount, onAdd, onImport, onClear }) => {
  const [input, setInput] = useState("")
  const fileInputRef = useRef<HTMLInputElement>(null)

  const handleAddClick = () => {
    const newAccounts = input
      .split("\n")
      .map((a) => a.trim())
      .filter((a) => a.includes(":"))
    if (newAccounts.length > 0) {
      onAdd(newAccounts)
    }
    setInput("")
  }

  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0]
    if (!file) return

    const reader = new FileReader()
    reader.onload = (e) => {
      const content = e.target?.result as string
      onImport(content, file.name)
    }
    reader.readAsText(file)
    event.target.value = "" // Reset input
  }

  return (
    <CollapsiblePanel title={`Accounts (${accountCount})`}>
      <textarea
        value={input}
        onChange={(e) => setInput(e.target.value)}
        placeholder="email:password..."
        className="w-full h-24 bg-gray-900 border border-gray-600 rounded-md p-2 text-sm font-mono focus:ring-cyan-500 focus:border-cyan-500"
      ></textarea>
      <div className="flex gap-2 mt-2">
        <button
          onClick={handleAddClick}
          className="flex-1 bg-cyan-500 hover:bg-cyan-600 text-white font-bold py-2 px-4 rounded-md text-sm transition-colors"
        >
          Add
        </button>
        <button
          onClick={() => fileInputRef.current?.click()}
          className="flex-1 bg-gray-600 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded-md text-sm transition-colors"
        >
          Import
        </button>
        <button
          onClick={onClear}
          className="bg-red-600 hover:bg-red-700 text-white font-bold p-2 rounded-md transition-colors"
          aria-label="Clear Accounts"
        >
          <Icon
            path="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0"
            className="w-5 h-5"
          />
        </button>
        <input type="file" ref={fileInputRef} onChange={handleFileChange} accept=".txt" className="hidden" />
      </div>
    </CollapsiblePanel>
  )
}

export default AccountManager
